...
if (file_exists($repoFile)) {
    ...
    if (class_exists('\model\repositorio\CategoriaFilmeRepositorio')) {
        ...
        if (method_exists($repositorio, 'isCategoriaUsed') && $repositorio->isCategoriaUsed($id)) {
            header("Location: /SistemaShopping_web1/src/view/administrativo/filme/horarios/horarios-filme-dashboard.php?erro=usada&erro_id=" . urlencode($id));
            exit;
        }
        try {
...
try {
    ...
    error_log("Erro ao excluir horarios-filme (PDO): " . $e->getMessage());
    header("Location: /SistemaShopping_web1/src/view/administrativo/filme/horarios/horarios-filme-dashboard.php?erro=sql");
    exit;
}
