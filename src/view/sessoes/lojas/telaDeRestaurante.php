...
// Removido carregamento da lista completa por padrão (não mostrar grid/card acima da ordenação)
$todasLojas = []; // agora vazia por padrão, usamos apenas a seção de "últimas lojas"

// Por padrão já mostrar os últimos 5 registros de restaurantes
...
if ($selectedShopLimit > 0) {
    try {
        // contar apenas restaurantes
        $totalLojas = $repositorio->contarLojasPorTipo(TipoLoja::Restaurante);
        $offset = ($selectedShopPage - 1) * $selectedShopLimit;
        // buscar apenas restaurantes paginados
        $ultimasLojas = $repositorio->buscarlojasPaginadasPorTipo(TipoLoja::Restaurante, $selectedShopLimit, $offset);
...
<head>
    ...
    <link rel="stylesheet" href="/SistemaShopping_web1/css/telaDeRestaurante.css">
</head>
<body>
        ...
        <section class="ultimas-lojas" style="padding:16px; border:1px solid #ddd; margin:16px;">
            ...
            <?php else: ?>
                ...
                <?php if ($totalShopPages > 1): ?>
                    <nav class="paginacao" aria-label="Paginação de restaurantes">
                        <?php
